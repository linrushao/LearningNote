...
- 一对多（one to many）/ 多对一（many to one）
  ...
  - 也可以通过内嵌文档来映射一对多的关系
    ...
    db.order.insert({
         list:["苹果","香蕉","鸭梨"],
         user_id:ObjectId("fawheifhaeoifhrei234")#"swk"的_id
    })
    
    db.order.insert({
         list:["西瓜","香蕉"],
         user_id:ObjectId("fawheioifhrei64634")#"zbj"的_id
    })
    ...
    db.stus.insert([
        {
        	name:"张三",
        	tech_ids:[
                ObjectId("fwehafioah123vfav13"),#张三的_id
                ObjectId("fafewfiewjf02434385")#李四的_id
            ]
        }
    ])
    ```
...
- Mongoose是一个对象文档模型(ODM)库，它对Node原生的MongoDB模块进行了进一步的优化封装并提供了更多的功能
...
3. 连接MongoDB数据库
   ...
   - mongoose.connect('mongodb://数据库的ip地址:端口号/数据库名'，{useMongoclient: truel});
...
6. schema和model
   ...
   //modelName 就是要映射的集合名，mongoose会自动将集合名变为复数（单数变复数，本身是复数就不变）
